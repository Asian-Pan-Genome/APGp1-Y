...
```
#### Extract and filter euchromatin maf: block length >= 1000bp && missing sites <= 0.2 && >=10 parsimony informative sites
```
...
```
Input: <phase2.chrYs.chm13.euchromatin.filter.maf>
Outputs: <MSY.TSPY_RM.fasta>, <MSY.TSPY_RM.fasta_stat> and <MSY.TSPY_RM.fasta.partition>
```
iqtree -s MSY.TSPY_RM.fasta -T AUTO -bb 1000 -m TEST --prefix 03.MSY.iqtree
```
### step4. 04.astral.sh
#### Generate ML tree for each block in MSY.euchromatin.filtered.maf_blocks, for example:
```
...
```
#### Run astral for coalescent tree
```
...
```
The <estimated_gene_trees.tree> is the block gene trees
The <estimated_species_tree.tree> is the iqtree.ml.tree generated above
