...
def draw(pandas_data, seque, v_max, output_name_1, output_name_2):
    # 将 seque 和 pandas_data 合并为一个 DataFrame
    seque_list = list(seque)  # 将序列转换为列表
    heatmap_values = pandas_data.values.flatten()  # 将热图数据展平为一维数组
    heatmap_values = heatmap_values[heatmap_values != -10] 
    # 确保 seque_list 和 heatmap_values 的长度一致
    if len(seque_list) != len(heatmap_values):
        raise ValueError("Length of seque and heatmap values do not match!")

    # 创建包含两列的 DataFrame
    df_output = pd.DataFrame({
        'Sequence': seque_list,
        'Heatmap_Value': heatmap_values
    })

    # 输出到文件
    output_file = output_name_1 + '_with_respect_to_' + output_name_2 + '_heatmap_data.txt'
    df_output.to_csv(output_file, sep='\t', index=False)
    print(f"\nHeatmap data saved to: {output_file}")
